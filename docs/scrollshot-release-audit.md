...
- External automatic runs expose `longScreenshotScrollDiagnostics`, including selected point/display metadata, adapter attempts, Windows UI Automation target metadata, wheel fallback details, and macOS Accessibility/CoreGraphics event output.
- The macOS external-auto smoke runs a ScreenCaptureKit `SCShareableContent` probe and records explicit environment-blocked skips only for diagnosed hosted-runner limits: missing Accessibility trust, or trusted CoreGraphics scroll events that are accepted but do not move the selected target region.
...
- OS-level external-window automatic flow: partially complete. Windows UI Automation plus wheel fallback is implemented and the Windows flow is verified with diagnostics. macOS ScreenCaptureKit probing, Accessibility preflight, and CoreGraphics scroll posting are implemented; the current hosted runner reports ScreenCaptureKit success and `AXIsProcessTrusted=true`, but the target region does not move after the posted scroll events, so the full macOS external-auto output remains environment-blocked rather than verified.
